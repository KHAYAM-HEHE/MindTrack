...
const hrLinks = [
  ...
  { to: "/hr/employees/new", label: "Create Employee" },
  { to: "/hr/verifications", label: "Verification Queue" },
];

function hrLinkIcon(label) {
  if (label === "Dashboard") return "grid_view";
  if (label === "Employee Management") return "group";
  if (label === "Create Employee") return "person_add";
  return "verified_user";
}

export default function HrShell({ title, subtitle, actions, children }) {
  ...
  const hrNavLinks = (onNavigate = () => {}) =>
    hrLinks.map((item) => {
      ...
      return (
        ...
        >
          <span className="material-symbols-outlined text-[20px]">{hrLinkIcon(item.label)}</span>
          {item.label}
        </Link>
      );
    });
  ...
  return (
    <div className="flex min-h-screen overflow-hidden bg-background text-on-background">
      <aside className="fixed left-0 top-0 hidden h-screen w-64 flex-col border-r border-outline-variant bg-surface-container-lowest p-4 shadow-sm md:flex">
        ...
        <nav className="flex flex-1 flex-col gap-1">{hrNavLinks()}</nav>
        ...
        <>
          ...
          <div className="fixed bottom-0 left-0 top-0 z-[70] flex w-[min(18rem,100vw-2rem)] flex-col bg-surface-container-lowest p-4 shadow-xl md:hidden">
            ...
            <nav className="flex flex-1 flex-col gap-1 overflow-y-auto pb-4">
              {hrNavLinks(() => setMobileNavOpen(false))}
            </nav>
        ...
        <header className="sticky top-0 z-40 flex items-center justify-between border-b border-outline-variant/60 bg-surface-container-lowest/80 px-4 py-3 shadow-sm backdrop-blur-md sm:px-6">
          <div className="flex min-w-0 flex-1 items-start gap-2">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-on-surface-variant">Signed in as {user?.role || "HR"}</p>
            </div>
          </div>
