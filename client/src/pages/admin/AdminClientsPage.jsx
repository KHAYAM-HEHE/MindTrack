...
export default function AdminClientsPage() {
  ...
  const load = useCallback((page = 1) => {
    if (!token) return;
    adminApi.listClients(token, { page, limit: 50, search }).then(setData).catch((e) => setError(e.message || "Failed"));
  }, [token, search]);
  ...
  return (
    <div className="space-y-6">
      ...
      <AdminSection
        title="Clients"
        subtitle="Manage client accounts and open detailed records."
        ...
        <AdminTable headers={["Name", "Email", "Status", "Action"]}>
          {data.items.map((u) => (
            <AdminRow key={u._id}>
              ...
              <AdminCell><Link to={`/admin/clients/${u._id}`} className="text-primary font-label-md hover:underline">Details</Link></AdminCell>
            </AdminRow>
          ))}
        </AdminTable>
