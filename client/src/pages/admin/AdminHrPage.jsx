...
export default function AdminHrPage() {
  ...
  const load = useCallback((page = 1) => {
    if (!token) return;
    adminApi.listHrUsers(token, { page, limit: 50 }).then(setData).catch((e) => setError(e.message || "Failed"));
  }, [token]);
  ...
  return (
    <div className="space-y-6">
      ...
      <AdminSection title="HR Management" subtitle="HR accounts (role HR). Only admins should change HR access.">
        <AdminTable headers={["Name", "Email", "Status", "Action"]}>
          {data.items.map((u) => (
            <AdminRow key={u._id}>
              ...
              <AdminCell><Link to={`/admin/hr/${u._id}`} className="text-primary font-label-md hover:underline">Details</Link></AdminCell>
            </AdminRow>
          ))}
        </AdminTable>
