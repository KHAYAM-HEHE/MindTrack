...
export default function EmployeeVerificationsPage() {
  ...
  const filtered = useMemo(() => {
    return employee.verifications.filter((item) => {
      const searchable = `${item._id} ${item.degree || ""} ${item.institution || ""}`.toLowerCase();
      return searchable.includes(query.toLowerCase());
    });
  }, [employee.verifications, query]);

  const onApprove = async (id) => {
    setProcessingId(id);
    setMessage("");
    try {
      await approveEmployeeVerification(id, token);
  ...
  const onReject = async (id) => {
    const reviewNotes = window.prompt("Add rejection notes");
    if (!reviewNotes) return;
    setProcessingId(id);
    setMessage("");
    try {
      await rejectEmployeeVerification(id, reviewNotes, token);
  ...
  return (
    <EmployeeShell
      ...
      subtitle="Review and process professional verification requests."
    >
      <div className="rounded-xl border border-outline-variant/30 bg-surface-container-lowest p-2 shadow-sm">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search by ID, degree, institution..."
            value={query}
          ...
          return (
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div>
                  <h3 className="font-semibold text-on-surface">{item.degree || "Credential submission"}</h3>
      ...
      {!filtered.length ? <p className="text-sm text-on-surface-variant">No verification records found.</p> : null}
    </EmployeeShell>
  );
}
