...
export default function PsychiatristNotificationsPage() {
  ...
  return (
    <PsychiatristShell title="Notifications" subtitle="Booking requests and schedule updates">
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mb-6 rounded-xl border border-outline-variant/30 bg-surface-container-low p-5">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <button
              type="button"
              className="rounded-xl bg-primary px-4 py-2 text-sm text-on-primary"
            ...
            <button
              type="button"
              className="rounded-xl border border-outline-variant px-4 py-2 text-sm text-on-surface hover:bg-surface-container-low"
            ...
            <Link
              ...
              className="inline-flex items-center rounded-xl border border-outline-variant px-4 py-2 text-sm text-on-surface hover:bg-surface-container-low"
            >
              Security & 2FA
            </Link>
          </div>
        ...
        <div className="space-y-3">
          {!loading && items.length === 0 ? (
            <div className="rounded-xl border border-outline-variant/30 bg-surface-container-lowest p-4 text-sm text-on-surface-variant">
              No notifications yet. When clients request appointments, entries appear here.
            </div>
          ) : null}
          {items.map((n) => {
            const isUnread = !n.readAt;
            const href = n.linkPath || "/psychiatrist/requests";
            return (
              <div
                key={n._id}
